...
/** Horizontal bars, one hue (same metric - probability - under different
 * scenarios, not distinct identities), rounded data-end, value at the tip. */
function renderProbabilityBars(container, rows) {
  container.innerHTML = "";
  if (rows.length === 0) {
    const empty = document.createElement("p");
    empty.className = "empty-state";
    empty.textContent = "No smart-buy scores yet";
    container.appendChild(empty);
    return;
  }
  ...
  for (const row of rows) {
    ...
    fill.style.width = `${Math.max(row.value, 2)}%`;
...
/** Diverging bars from a center 0% line - this game's value vs its cluster's
 * average, as a % difference. Blue = above average, red = below; a neutral
 * gray baseline at center. Not a value judgment (above isn't "better"),
 * just distance from the cluster's typical profile. */
function renderDivergingBars(container, rows) {
  container.innerHTML = "";
  if (rows.length === 0) {
    const empty = document.createElement("p");
    empty.className = "empty-state";
    empty.textContent = "Not enough discount history to cluster yet";
    container.appendChild(empty);
    return;
  }

  const maxAbs = Math.max(...rows.map((r) => Math.abs(r.pctDiff)), 10);
  const tooltip = makeTooltip(container);

  const list = document.createElement("div");
  list.className = "diverging-rows";
  for (const row of rows) {
    const rowEl = document.createElement("div");
    rowEl.className = "diverging-row";

    const label = document.createElement("div");
    label.className = "diverging-row-label";
    label.textContent = row.label;

    const track = document.createElement("div");
    track.className = "diverging-row-track";
    const center = document.createElement("div");
    center.className = "diverging-row-center";
    track.appendChild(center);

    const bar = document.createElement("div");
    bar.className = "diverging-row-bar";
    const magnitude = (Math.abs(row.pctDiff) / maxAbs) * 50;
    bar.style.width = `${magnitude}%`;
    bar.style.background = row.pctDiff >= 0 ? "var(--diverging-pos)" : "var(--diverging-neg)";
    if (row.pctDiff >= 0) {
      bar.style.left = "50%";
    } else {
      bar.style.right = "50%";
    ...
    track.addEventListener("pointermove", (event) => {
      const rect = container.getBoundingClientRect();
      const sign = row.pctDiff >= 0 ? "+" : "";
      showTooltip(tooltip, container, event.clientX - rect.left, event.clientY - rect.top, [
        { strong: `${sign}${row.pctDiff.toFixed(0)}%` },
        { text: `vs. cluster average (${row.label})` },
      ]);
    });
    track.addEventListener("pointerleave", () => hideTooltip(tooltip));

    const value = document.createElement("div");
    value.className = "diverging-row-value";
    const sign = row.pctDiff >= 0 ? "+" : "";
    value.textContent = `${sign}${row.pctDiff.toFixed(0)}%`;
