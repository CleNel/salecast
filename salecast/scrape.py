...
def scrape_game(
    ...
    """Fetches app_id's current price/discount from Steam's appdetails and
    inserts a row into price_history (source='daily_scrape'). Returns rows
    inserted (0 if the app has no store listing or isn't currently priced,
    e.g. free-to-play)."""
    ...
    cursor = conn.execute(
        ...
        VALUES (?, ?, ?, ?, 'daily_scrape')
        """,
        (app_id, date.today().isoformat(), details["price"], details.get("discount_pct")),
    )
    conn.commit()
